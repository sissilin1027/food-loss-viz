...
first_title = 'Loss to Production Ratio (%) in ' + dfy[0][0]
traces = []
buttons = []
for i, d in enumerate(dfy):
    ...
    buttons.append(dict(label=name,
                        ...
                        args=[{"visible": visible},
                              {"title": 'Loss To Production Ratio (%) in ' + f"{name}"}]))
...
app.layout = html.Div([
    ...
    html.P("“If food loss and waste were a country, it would be the third biggest source of greenhouse gas emissions” - Inger Anderson, United Nations Environment Program",
           style={'text-align': 'center', 'font-family': 'courier new', 'margin-bottom': '50px', 'font-size': '18pt','width': '60%','padding-left': '20%','padding-right':'20%'}),
    html.Br(),
    ...
    dcc.RadioItems(
        id='cluster',
        options=[
            {'label': '10 countries with the highest food loss to production ratio (%)', 'value': '10 countries with the highest food loss to production ratio (%)'},
            {'label': '10 countries with the lowest food loss to production ratio (%) ', 'value': '10 countries with the lowest food loss to production ratio (%)'},

        ],
        value='10 countries with the highest food loss to production ratio (%)',
...
def update_cluster(topic):
    ...
    text_high = html.Div([
        ...
        html.Table([
            html.Tr([html.Th('Countries'),
                     html.Th('Perishable Clusters'),
                     html.Th('Non-Perishable Clusters'),
                     html.Th('Total Clusters')],style={"background-color": "white"}),
    ...
    if topic == '10 countries with the highest food loss to production ratio (%)':
        return text_high
    elif topic == '10 countries with the lowest food loss to production ratio (%)':
        return text_low
